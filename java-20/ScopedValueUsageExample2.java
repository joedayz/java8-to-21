public class ScopedValueUsageExample2 {
...
class Worker {
	...
	public void execute() {
		...
		System.out.println("shared value from main: " + ScopedValueUsageExample2.MAIN_SCOPE.get());
		...
		ScopedValue.where(WORKER_SCOPE, "message from worker")
			.run(() -> {
				...
				var messageFromMain = ScopedValueUsageExample2.MAIN_SCOPE.get();
		...
		ScopedValue.where(ScopedValueUsageExample2.MAIN_SCOPE, "message from worker over main")
			.run(() -> {
				// the outmost scope will still works
				var rebindedMessage = ScopedValueUsageExample2.MAIN_SCOPE.get();
		...
		System.out.println("shared value from main after all scopes: " + ScopedValueUsageExample2.MAIN_SCOPE.get());
	}
}